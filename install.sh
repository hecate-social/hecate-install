...
setup_gitops_repo() {
    section "Setting up Local GitOps Repository"
    ...
    # Create kustomization for hecate namespace
    create_hecate_manifests

    # Create flux source pointing to local repo
...
EOF
    ...
    git add -A
    git commit -m "Initial hecate gitops setup" 2>/dev/null || true

    ok "GitOps repository created at ${GITOPS_DIR}"
}
...
create_hecate_manifests() {
    info "Creating Hecate manifests..."

    # Namespace
    cat > hecate/namespace.yaml << EOF
apiVersion: v1
kind: Namespace
metadata:
  name: hecate
...
EOF
    ...
    cat > hecate/headless-service.yaml << EOF
apiVersion: v1
kind: Service
metadata:
  name: hecate-daemon-headless
  namespace: hecate
spec:
  clusterIP: None
  selector:
...
EOF
    ...
    # DaemonSet for hecate-daemon
    cat > hecate/daemon-daemonset.yaml << EOF
apiVersion: apps/v1
kind: DaemonSet
metadata:
  name: hecate-daemon
    ...
    spec:
      ...
      containers:
        - name: daemon
          ...
          env:
            - name: HECATE_API_HOST
              value: "0.0.0.0"
            - name: HECATE_API_PORT
              value: "4444"
            - name: HECATE_SOCKET_PATH
              value: "/run/hecate/daemon.sock"
            - name: HECATE_BOOTSTRAP
              value: "boot.macula.io:443"
            - name: HECATE_REALM
              value: "io.macula"
            - name: OLLAMA_HOST
...
EOF
    ...
    # Kustomization for hecate
    cat > hecate/kustomization.yaml << EOF
apiVersion: kustomize.config.k8s.io/v1beta1
kind: Kustomization
namespace: hecate
resources:
...
deploy_hecate() {
    ...
    # Direct apply for immediate deployment
    kubectl apply -k hecate/
